...
interface TermsAgreementProps {
  onAgreementComplete: (isAllAgreed: boolean) => void;
}

export const TermsAgreement = ({ onAgreementComplete }: TermsAgreementProps) => {
  const [state, dispatch] = useReducer(reducer, { terms1: false, terms2: false });

  const allChecked = state.terms1 && state.terms2;

  useEffect(() => {
    onAgreementComplete(allChecked);
  }, [allChecked, onAgreementComplete]);
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="terms1" className="flex cursor-pointer text-lg text-gray-900">
          <input
            id="terms1"
            type="checkbox"
        ...
        <AlertDialog>
          ...
          <AlertDialogContent className="w-[671px]">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-[32px] font-bold text-gray-900">이용약관</AlertDialogTitle>
              <AlertDialogDescription className="font-normal text-gray-600">
                <span className="text-xl text-gray-900">홈페이지 이용약관</span>
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="terms2" className="flex cursor-pointer text-lg text-gray-900">
          <input
            id="terms2"
            type="checkbox"
        ...
        <AlertDialog>
          ...
          <AlertDialogContent className="w-[671px]">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-[32px] font-bold text-gray-900">개인정보 수집 및 이용</AlertDialogTitle>
              <AlertDialogDescription className="font-normal text-gray-600">
                <span className="block text-xl text-gray-900">개인정보 수집 및 이용</span>
                {`개인정보보호법에 따라 인싸이더에 회원가입 신청하시는 분께 수집하는 개인정보의 항목, 개인정보의 수집 및 이용목적, 개인정보의 보유 및 이용기간, 동의 거부권 및 동의 거부 시 불이익에 관한 사항을 안내 드리오니 자세히 읽은 후 동의하여 주시기 바랍니다.\n 제 1조 (목 적)\n 이 서비스 이용약관(이하 “약관”이라 합니다)은 네이트커뮤니케이션즈㈜(이하 “회사”라 합니다)가 제공하는 서비스와 관련하여 회사와 이용 고객(또는 회원) 간에 서비스의 이용 조건 및 절차, 회사와 회원 간의 권리, 의무 및 책임 사항 기타 필요한 사항을 규정함을 목적으로 합니다.`}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
